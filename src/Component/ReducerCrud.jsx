...
const ReducerCrud = () => {
    ...
    const reducer = (state, action) => {
        switch(action.type){
            ...
            case "delete":
                // let data = state.splice(action.i, 1);
                // let data = [...state];
                // data.splice(action.i, 1);
                // state.filter((x,index) => index != action.i);
                return state.filter((x,index) => index != action.i);;

            case "update":
                let data = [...state];
                data.splice(action.i, 1, action.obj);
                return data;
    ...
    const [ary, dispatch] = useReducer(reducer, JSON.parse(localStorage.getItem("ary")) || [])
  ...
  return (
    <>
        ...
        <form action="" className="shadow-lg p-3 rounded-3" id="form">
            ...
            <input type="file" name="profile" className='mt-2' onChange={handleChange}/> <br />
